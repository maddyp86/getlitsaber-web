...
- **Status tracking = HubSpot Tickets pipeline.** Create a "Rebate" ticket pipeline with stages **Pending → Approved → Complete**. One ticket per submission, opened by a HubSpot workflow triggered on the rebate form submission (associate the ticket to the contact; carry order #, platform, post URL onto the ticket). Ops manually advances the stage: verify the post is real + public → move to Approved → issue the $5 refund in Shopify against the order → move to Complete. "Complete" means the Shopify refund is actually issued.
- **Confirmation email = HubSpot workflow on Approved/Complete.** A workflow sends the "your $5 is on the way" email when the ticket reaches Approved (or Complete) — same architecture as the Gold-waitlist confirmation (HubSpot owns the email; no backend built here). Email copy must follow BRAND.md (no exclamation marks, no em-dashes, no superlatives).
