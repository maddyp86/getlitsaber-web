...
**2026-08-06 — The shipping experiment's zero was never a missing property. Checking out is what disqualified the buyer.** The PostHog inbox had been re-reporting the same finding for three weeks: the `Single-unit shipping surcharge` primary metric sums `purchase.contribution`, `purchase` has no `contribution` property, so the metric reads 0. That was true when it was first written on 2026-07-14, and it stopped being true on 2026-07-18 when the constants landed. Every re-validation after that restated the original diagnosis without re-checking it, so a stale root cause survived five scout passes while the real one went unexamined.

**The tell was in the metrics that were NOT contribution.** `Subtotal per exposed`, `Units per exposed`, and `Shipping collected per exposed` all read 0 too, and those properties have always been on the event. A missing property can zero out one metric. It cannot zero out three that are present and populated. So whatever was wrong sat upstream of the properties, in the join from exposure to purchase.

**Root cause: the flag buckets on `distinct_id`, and we change the `distinct_id` at checkout.** `identifyByEmail()` fires from the cart drawer and the cart page at the moment a shopper proceeds to checkout, swapping `distinct_id` from `$device_id` to their email. `ensure_experience_continuity` is off on the flag, so posthog-js re-requests flags under the new id and re-buckets from scratch. Of the visitors who were exposed under both an anonymous and an identified id, three in four came back with the *other* arm. Two arms on one person means PostHog files them as `$multiple` and drops them from the analysis entirely.

**Which makes the failure mode self-selecting, and that is the part worth remembering.** Identification happens at checkout, so the only visitors who get re-bucketed are the ones about to buy. The experiment was systematically excluding exactly the population it needed to measure. "Purchase conversion has zero observed steps" was not a sample-size problem; the converters were being disqualified by the act of converting. The control arm's sole buyer was `$multiple`, so control summed to a clean 0 across every metric, which reads exactly like an instrumentation gap and is why the wrong diagnosis stuck for so long.

**Three smaller defects surfaced alongside it, all in the same attribution path.** (a) Cart attributes are only written at `cartCreate`, so a returning visitor whose cart persisted arrived at the webhook with no visitor id and fell back to a synthetic `order_<id>` that can never join a person. (b) Those same orders lost `subtotal`, and `parseFloat(x) || 0` turned "absent" into 0, so `contribution` computed costs against no revenue and emitted a *negative* number straight into the primary metric. A missing property contributes nothing; a fabricated loss actively subtracts. (c) `_shipping_variant` defaulted to `"control"` when flags had not loaded, filing unbucketed orders under control and diluting the arm.

**Fix.** The arm is now sticky per device (first resolved value persisted, read through from then on), so a mid-session re-bucket cannot move a price a shopper was already quoted or mislabel the arm frozen onto the cart. `hydrate()` backfills attribution onto carts that never got it. `parseMoney` distinguishes absent from zero, and `contribution` is omitted rather than invented when subtotal is unknown. The purchase event now carries `$feature/single-unit-shipping-surcharge`, so the result can be read off the purchase itself instead of depending on how the exposure happened to be recorded. Non-answers ("unknown", "unresolved") are omitted rather than counted as control.

**Still open, and both are calls for Matt, not code.** `ensure_experience_continuity` should go on the flag — it is the PostHog-side half of the sticky fix and the only thing that stops the exposure events themselves from flapping. And the data collected so far cannot be salvaged: the control arm has no usable observations, and the surcharge arm's entire signal is one order counted three times from before the idempotency fix. The experiment wants a reset once the fix is live, not a longer run.

**2026-08-05 — One Shopify order, three `purchase` events: the webhook had no idempotency.** Order #1040 showed up three times in PostHog. All three were `posthog-node`, so all three were `app/api/webhooks/orders/route.ts` running three times — not a client double-fire, and not a duplicated webhook subscription either, since #1040 was the only affected order out of 18 in 120 days (a misconfigured subscription would hit every order).

**Root cause: Shopify delivers webhooks at least once, and the handler treated every delivery as a new purchase.** The timeline gave it away. Shopify created and charged the order at `18:23:45`; the captures landed at `18:24:04.377`, `18:24:04.390` (+13ms), and `18:25:03.276` (+59s). A 19-second gap to the first capture on a serverless route is a cold start, and Shopify's webhook budget is **5 seconds** — it gave up waiting, marked the delivery failed, and retried twice. Each retry still ran the handler to completion, and each completion fired a fresh event. The two captures 13ms apart are the tell: that is not a retry sequence, that is two deliveries executing concurrently.

**The diagnostic that settled it:** Supabase had exactly ONE row for the order while PostHog had three. Same handler, three runs, one row. The DB write was already idempotent through the unique index on `shopify_order_id`; the PostHog capture was the only side effect with no guard. Reading the two artifacts against each other located the bug faster than reading the code did — the same "read what actually landed, not what the code intended" move that cracked the `device_type` pipe in 5.5.

**Impact was not cosmetic.** The duplicate sat in the treatment arm of the live shipping-surcharge experiment, whose primary metric is `sum` of `contribution` on `purchase` (a mean-type metric), so the order counted three times: contribution read $269.15 against a true $157.49, or $4.08 per exposed user against $2.39. The primary metric was overstated by 71% in the arm the experiment was trying to prove. The "Purchase conversion" secondary is a funnel on unique users and was untouched — a useful reminder that duplicate events corrupt sum-based metrics and leave count-distinct ones alone.

**Fix (PR #55) is two guards, because each alone has a hole.** (a) `insertOrder` now reports whether THIS call created the row — `ignoreDuplicates: true` compiles to `INSERT ... ON CONFLICT DO NOTHING`, so the added `.select()` returns the row on a first delivery and an empty set on a redelivery, and the PostHog capture is gated on it. The unique index is what arbitrates, which is why it holds for the two genuinely concurrent deliveries: they race in Postgres and exactly one wins. A select-then-insert check would have failed that case. (b) The event `uuid` is derived from the order id and the `timestamp` from the order's `created_at`, so anything slipping past (a) carries an identical dedupe key (PostHog treats matching `uuid` + `event` + `timestamp` + `distinct_id` as duplicates) and collapses into the first event. Guard (a) is deterministic; guard (b) does not depend on our own DB being reachable.

**The trade-off, named rather than hidden:** guard (a) means a successful insert followed by a failed PostHog flush loses the event, because the retry now skips it. Logged with the order id, and `orders.raw` holds the payload, so it is replayable. Accepted deliberately over the alternative of dropping guard (a) and trusting PostHog's dedupe alone.

**Cleanup: the duplicates cannot be deleted.** PostHog has no supported event deletion, and deleting the person would take the legitimate purchase with it. So the correction is at read time: a saved view `purchases_deduped` groups `purchase` by `order_id` at one-row-per-order grain, keeping a `deliveries` column so duplicates stay auditable instead of silently vanishing. All revenue and order-count reporting should read the view, not the raw event. **Known limitation:** PostHog experiment metrics take an EventsNode and cannot point at a warehouse view, so the experiment UI still shows the inflated $4.08 — the correction has to be applied by hand when reading it.

**Still open:** the 5-second budget is the underlying trigger, and this fix makes retries harmless without stopping them. The clean fix is returning 200 immediately after HMAC verification and deferring the Supabase write plus PostHog flush via `waitUntil` from `@vercel/functions` — a new dependency, so it needs its own decision. This matters past duplicates: Shopify removes a webhook subscription after 19 consecutive failed deliveries in 48 hours, so a chronically slow endpoint can silently lose the subscription altogether.
...
| P7-2 | "Post-launch, the contact form started getting spammed. Root cause was a bootstrapping tradeoff nobody logged at the time: HubSpot's native reCAPTCHA had been turned OFF so the custom form → /api/contact → HubSpot Forms API v3 proxy could submit at all (HubSpot's built-in captcha only guards HubSpot-hosted/embedded forms, not server-to-server Forms API submissions). Disabling it removed the only real bot barrier and left just a honeypot + in-memory rate limiter, which bots walked through. Fix: added Cloudflare Turnstile — client widget issues a single-use token, the API route calls siteverify BEFORE forwarding to HubSpot. Chose Turnstile over reCAPTCHA v3/hCaptcha for the free-unlimited + no-image-puzzle + privacy posture. Two deliberate fail directions: fail-OPEN when the secret is unset (local dev / rollback never blocked), fail-CLOSED when it's set but the token is bad or Cloudflare is unreachable (keep spam out even during a siteverify outage). Reusable seam left behind: `lib/security/turnstile.ts` + `<TurnstileWidget>`." | `security`, `integration-depth`, `debugging` |
| P7-3 | "Extended Turnstile from contact to the footer email banner and the wholesale form. The non-obvious catch: the footer banner (`EmailSignupBanner`) and the Gold/Future-Drops waitlist modals (`WaitlistForm`) BOTH POST to the one `/api/subscribe` route. Gating that route server-side without adding the widget to WaitlistForm too would have silently broken the modals the moment the secret went live (fail-closed rejects a tokenless submit). So 'protect the footer' actually meant 'wire every consumer of the shared route.' Also gave the compact/high-conversion forms `appearance=\"interaction-only\"` (invisible unless Cloudflare challenges) vs the full visible widget on the big contact/wholesale forms — one component, two postures by prop. Debugging lesson banked: after a burst of edits the Next dev server pinned a stale SWC 'Unexpected token section' error from a half-saved intermediate state and kept re-emitting it across hard reloads even though tsc passed, the JSX balanced, and SSR fetches returned real rendered markup. The tell that it was wedged-HMR not a real defect: SSR uses the same SWC, so rendered content in the HTML is proof the file parses. Restarting the dev server cleared it. Don't trust a persistent dev-overlay error over a clean tsc + real SSR output." | `debugging`, `integration-depth` |
---

### Shipping surcharge A/B · data piping (2026-07-13) ✅ piping built · flag + Shopify Function deferred

Brief: `docs/shipping-surcharge-ab.md`. Test hypothesis: charge $5.99 shipping on single-unit orders, free on 2+, and win on contribution per exposed visitor (not conversion rate). This pass builds and QA-proves the data piping only · creating the PostHog flag/experiment and the Shopify delivery Function are out of scope and deferred until the QA gate passes.

**Discovery (resolved from the codebase, not guessed):**
- Purchase webhook is a **Next.js API route**, `app/api/webhooks/orders/route.ts` (not n8n) · verifies HMAC, upserts Supabase, fires PostHog server-side via `posthog-node`. So the property additions are a code change here.
- Purchase event name is `"purchase"`, captured at route.ts ~L153. Properties assembled L79 to L119.
- Flag `single-unit-shipping-surcharge` confirmed absent in code (grep · zero hits).
- **Contract deviation banked:** the brief specified a new `_ph_distinct_id` cart attribute, but the repo already stamps and reads `posthog_distinct_id` for the exact same server-side attribution (store.ts ~L225 → route.ts ~L89). Reused the existing stitch · did NOT add `_ph_distinct_id`. Confirmed with Matt.

**Backend (server-side, landed first):**
- Extended `ShopifyOrder` to read `current_subtotal_price`, `total_shipping_price_set.shop_money.amount` (fallback: sum of `shipping_lines[].price`), and `note_attributes._shipping_variant` (same read path as `channel_type`, defaults to `"unknown"`).
- Added `subtotal`, `shipping_amount`, `shipping_variant` to the `purchase` capture props.
- Added `shipping_variant` (text) + `shipping_amount` (numeric) to Supabase `orders` · TS interface in `supabase/client.ts` and the `insertOrder` call · migration `supabase/migrations/20260713163135_005_add_shipping_ab.sql`. **Migration file written · not yet applied to the live DB** (device_type precedent was a manual SQL-Editor run).

**Frontend:**
- `lib/experiments/useShippingVariant.ts` · reads the flag off the **global posthog singleton** via `getFeatureFlag` + `onFeatureFlags`, not `useFeatureFlagVariantKey`. Reason: `app/providers.tsx` inits one global instance and does not mount the `posthog-js/react` context provider, and the brief forbids touching init. Same exported contract: `'control' | 'surcharge' | undefined`. Also exports `readShippingVariant()` for non-React callers.
- Freeze at cartCreate: `store.ts` stamps `_shipping_variant` once in the cartCreate attributes block (underscore-prefixed → suppressed from checkout UI, visible to the Order API). Only stamped there, never on cartLinesAdd, so it cannot flip mid-cart.
- `lib/shipping.ts` · `getDisplayShipping(units, variant)` + `formatDisplayShipping` · display-only mirror. Verified against all arms via tsx: control single → FREE, surcharge single → $5.99, 2+ either arm → FREE, unknown → "Calculated at checkout".
- `components/product/ShippingUpsellNudge.tsx` · shared nudge, renders only for `surcharge` at `units < 2`, else clean null. Wired into PDP (`BundleAndCTA`, taps → select two-pack), cart page (`CartPageBody`, both layouts, taps → `updateQty(silverLine, 2)`) and drawer (`CartDrawer`). Cart shipping rows now mirror `getDisplayShipping`.

**Verification this pass:** tsc strict clean · eslint clean · null path renders on PDP with no new console errors (only the pre-existing local `NEXT_PUBLIC_SHOPIFY_STORE_DOMAIN not set` cart error). Surcharge visual path and the live QA-gate insight (single-unit surcharge purchases must all read `shipping_amount = 5.99`) require the deployed env + the flag + the Shopify Function, and are deferred per the brief.

**Transparency pass (Matt's call, same day) — surcharge is stated up front, not just at checkout:**
- Migration applied to the live DB (`getlitsaber-web-live`, ref `otlinkrztvxwhlefqaro`) via Supabase MCP · `orders.shipping_variant` (text) + `shipping_amount` (numeric) confirmed present.
- **$5.99 folded into the displayed cart Total** (page + drawer): `displayTotal = subtotal + (shippingCost ?? 0)`. When the flag is unresolved, falls back to subtotal (no jump). Reverses the earlier "total excludes shipping" stance · Matt chose the transparent version.
- **Announcement bar** `components/layout/AnnouncementBar.tsx` · surcharge arm only ("Free shipping when you buy 2 or more"). Control ships singles free too, so the bar would mislead them · gated to `surcharge`. Layout: the bar sets `.has-shipping-bar` on `<html>` → `--shipping-bar-h: 40px`; the fixed navbar (`top: var(...)`) and `main` (`padding-top: var(...)`) both shift down by exactly that, so no per-page offset edits. Default 0 → zero gap / no CLS for control / SSR. Verified in-browser: navbar + content shift 40px in lockstep, 0 when absent.
- **PDP shipping note** in `BundleAndCTA` (surcharge arm) states the shipping for the current selection: "Shipping: $5.99 · Free when you buy 2 or more" for a single, "Shipping: FREE" at 2+.
- Gating rationale banked: everything surcharge-visible (bar, folded total, PDP note, nudge) reads `undefined` until the flag exists, so this whole layer is invisible until the experiment launches · consistent with piping-first.

**Companion half — Shopify delivery Function (LOGIC BUILT 2026-07-13, pending deploy):** Brief `shopify-delivery-function.md`. This storefront repo only stamps `_shipping_variant` and mirrors the display · it CANNOT charge. The actual $5.99 is enforced by a Shopify Function that hides the wrong rate per bucket: `itemCount >= 2` → hide $5.99; surcharge single → hide $0; control/unknown single → hide $5.99. Hard rule: never hide every option in a group (Shopify rates-don't-reappear bug) · function skips a group when nothing would remain. Depends on two flat rates in the US zone ("Free Shipping" $0.00, "Standard Shipping" $5.99).

- **Location:** `~/Documents/GitHub/litsaber-shopify-functions/litsaber-checkout-functions` (separate Shopify CLI app, separate deploy · app handle `shipping-surcharge-gate`).
- **Target correction (important):** scaffold used the NEWER target **`cart.delivery-options.transform.run`** (API `2026-04`), NOT the brief's `purchase.delivery-customization.run`. Result shape is `{ operations: [Operation!]! }` where `Operation` is `@oneOf` · the hide op is **`{ deliveryOptionHide: { deliveryOptionHandle } }`** (brief's `{ hide: {...} }` was stale). Export `cart-delivery-options-transform-run`, fn `cartDeliveryOptionsTransformRun`.
- **Rate matching:** `CartDeliveryOption.cost` (MoneyV2) IS exposed → matched by cost in cents (0 vs 599), no title fallback needed.
- **Files written (JS `@ts-check`):** rewrote `src/cart_delivery_options_transform_run.graphql` (fetches `cart.lines.quantity`, `cart.attribute(key:"_shipping_variant").value`, `cart.deliveryGroups.deliveryOptions{handle,title,cost.amount}`) + `src/cart_delivery_options_transform_run.js` (logic). Replaced the starter `no-operations.json` fixture (its input no longer validates) with 5 fixtures: control-single, surcharge-single, two-pack-surcharge, missing-attribute (→ control), surcharge-single-only-free (never-hide-all → `[]`). All 5 pass a deterministic run of the real function; the vitest wasm build (`npm install` + `npm test`) + deploy are Matt's to run (CLI not installed here).
- **Matt still runs:** `npm install` in the extension → `npm run typegen` (regenerate `generated/api.ts` for the new input query) → `npm test` → `shopify app deploy` → create + activate a delivery customization pointing at the function (admin or `deliveryCustomizationCreate`).
- Watch item: confirm the underscore key `_shipping_variant` actually reaches the function at real checkout · if stripped, storefront stamps a duplicate non-underscore fallback key.

**Commit plan (locked):** storefront changes (piping + transparency) stay UNCOMMITTED for now. Everything ships to the `staging` branch together — storefront + Shopify Function + the PostHog flag — for one full end-to-end test, once the Shopify side and flag are set up. Then the QA gate insight (surcharge singles must all read `shipping_amount = 5.99`) runs before the experiment launches.

**Activation-pass findings (2026-07-13, function deployed):**
- **PostHog flag already exists** — id `759944`, key `single-unit-shipping-surcharge`, variants `control`/`surcharge` 50/50. It was auto-created by a PostHog **Experiment** (`383527` "Single-unit shipping surcharge", `is_running: false`). Flag is currently `active: false`, release group at 100% rollout. So it's in experiment-DRAFT state, not the "0% for testing" state. To test both arms without exposing real users: keep the flag inactive and use a client-side PostHog override in the tester's browser (the storefront stamps the cart attribute from the browser-side flag read, so an override flows through to the Function), OR add a tester-targeted 100% condition. Do NOT just flip `active:true` — that launches the 50/50 split to all traffic.
- **Shopify delivery customization CANNOT be activated via MCP** — the "Shopify Claude Connector App" lacks `read/write_delivery_customizations`, and a delivery customization must be created by the app that owns the function anyway. Activation is a Matt task: admin (Settings › Shipping and delivery › Delivery customizations) or the function app's own Admin API.
- **Shipping rates — RESOLVED 2026-07-13.** Matt confirmed the purchasable Silver variant ships from the **Warehut 3PL Fulfillment** profile / North America zone. That zone had only "Free Shipping (USPS Ground)" $0.00, so via `deliveryProfileUpdate` (MCP, write_shipping) added **"Standard Shipping" $5.99**. The zone now holds exactly two rates: Free $0.00 (→ 0¢) + Standard $5.99 (→ 599¢), matching the Function's `FREE_CENTS`/`STANDARD_CENTS`. The General profile's $8/$0/$15 rates serve the other (Gold) variant and don't apply to Silver, so nothing leaks. Profile id `88475861199`, location group `89497043151`, zone `309468070095`.
- **Remaining for Matt:** (1) activate the delivery customization in admin pointing at `shipping-surcharge-gate`; (2) give go-ahead to commit storefront → `staging`; (3) test both arms via a PostHog browser override, verifying the $5.99/Free charge, that `_shipping_variant` reaches the Function, and that the purchase event + Supabase get `shipping_amount`; (4) QA gate insight.

**QA GATE PASSED 2026-07-14.** Three live staging test orders, verified in both PostHog (test project 445005) and Supabase (`getlitsaber-web-live`), agree exactly: #1030 surcharge single → `shipping_amount` 5.99; #1031 control single → 0; #1032 surcharge two-pack → 0. `subtotal` reads 49.99 for the retained single in both arms (the isolation the primary metric needs). All three new event props (`subtotal`, `shipping_amount`, `shipping_variant`) present; Supabase `shipping_variant`/`shipping_amount` populated. The charge-enforcement Function and the storefront piping are proven end to end.

**PRE-LAUNCH GOTCHA — PostHog project routing.** Staging posts purchase events to project **445005 "Litsaber - Test"**, but the flag + experiment `383527` live in **480206 "Litsaber - Live"**. Before launching, confirm PRODUCTION env points at 480206 (both the client `NEXT_PUBLIC_POSTHOG_TOKEN` that reads the flag AND the webhook that captures `purchase`), or the live experiment will not see its own conversion events. Test data staying in 445005 is correct and should NOT be launched from. Note: client and server use the SAME `NEXT_PUBLIC_POSTHOG_TOKEN`, so they can't diverge — confirming one confirms both; Vercel Preview (staging) uses the 445005 token, Production should use 480206.

**Surcharge transparency UI revamp 2026-07-14 (uncommitted).** Three display-only changes, all still gated to the existing surcharge variant (reused `useShippingVariant`). (1) Deleted the announcement bar entirely: removed `AnnouncementBar.tsx`, its `SiteChrome` mount, the Navbar `top` var, and the `--shipping-bar-h`/`.has-shipping-bar`/`main padding-top` block in globals.css. (2) PDP `BundleAndCTA`: deleted the `ShippingUpsellNudge` box and the redundant "Shipping: $5.99 · Free…" line; recolored the SAVE badge to magenta (#EC5793 text on rgba(236,87,147,0.12)); added a cyan FREE SHIPPING badge on the 2+ tiles (twopack AND more), surcharge-only. Both badges Space Mono 10.5px / 0.5px / 5px radius / 3px 8px. Verified at 380px: badges wrap under the title, price uncrowded. (3) Cart page + drawer: replaced the nudge box with `ShippingUnlockMeter` (new component) under the Shipping row — 6px #14142c track, cyan fill = itemCount/2 (→50% for a single), label "ADD 1 TO UNLOCK FREE SHIPPING · SAVE $5.99". Shows only surcharge && itemCount<2. Deleted the now-orphaned `ShippingUpsellNudge.tsx`. Token: corrected `magenta-alpha-10` from rgba(255,0,229,.1) to rgba(236,87,147,.1) (the only #FF00E5-based token). Left 4 unrelated scattered `#FF00E5` literals in policies/TikTok untouched (out of scope). tsc + lint clean; grep-confirmed zero refs to the removed pieces. NOTE: `lib/shipping.ts` (getDisplayShipping/formatDisplayShipping) is still used by the cart Shipping row + folded total, so kept. The static PDP TRUST_LINE still says "FREE US SHIPPING" in all arms — flagged, not changed (out of scope).

**Unlock-meter tappable + copy 2026-07-14 (uncommitted).** Three follow-ups to `ShippingUnlockMeter`: (1) copy now "ADD 1 MORE TO UNLOCK FREE SHIPPING · SAVE $5.99" (added the missing MORE). (2) The whole meter (track+label) is now a `<button>` with `aria-label="Add one more to unlock free shipping"`, cursor-pointer, and a hover affordance (brightened fill + subtle cyan ring). On click it calls `onUnlock`, which the cart page and drawer supply as `updateQty(silverLine.id, 2)` — reuses the existing cart-quantity action (store `updateQty` → `cartLinesUpdate`), targets the Silver line by GID `gid://shopify/ProductVariant/45098118316239` (const `SILVER_VARIANT_ID`, `items[0]` fallback), no navigation. After the mutation itemCount hits 2 and the meter self-hides via its `itemCount < 2` guard; shipping recalculates free. (3) Added a thin `#1a1a30` top divider on the meter (borderTop + paddingTop) so it groups under the Shipping row without colliding with Estimated Tax; still directly under Shipping, not under Total. Applied in both `CartPageBody` (mobile + desktop summaries) and `CartDrawer`. tsc + lint clean. Live click-through (qty→2, free shipping, meter hides, total = two-pack) is a staging check — local dev has no Shopify cart or PostHog arm to render it.

**Contribution as native primary metric 2026-07-15.** Acted on Signals Scout report `019f628b` (valid: the primary `Subtotal per exposed` omits shipping recovery + costs, so it under-credits the surcharge and could read flat/down while true contribution is up). PostHog experiment metrics can only aggregate ONE property, so added a baked `contribution` number to the `purchase` event in the webhook: `contribution = subtotal - 13.33*item_count + shipping_amount - 7.50` (COGS $13.33 landed/unit, postage $7.50 = midpoint of the $6-9 range, both documented constants in route.ts). Sanity: control single $29.16, surcharge single $35.15, surcharge two-pack $55.83. Repointed experiment `383527` PRIMARY → `Contribution per exposed` (mean); demoted Subtotal to a secondary alongside conversion/units/shipping. Aligned insight `Z3bd3MOm` to the same formula (recomputes from subtotal/units/shipping so it works pre-deploy AND cross-checks the event's baked value). **The webhook change is UNCOMMITTED — needs commit → merge to main → deploy before the native `contribution` property flows; until then the experiment's primary reads ~0 but the insight already shows the number.** Left the report state alone (pipeline-managed); Matt can mark it resolved. Changed the experiment metric only ~a day after launch (~8 exposures), so negligible pre-change data.

**EXPERIMENT LAUNCHED 2026-07-14 18:25 UTC.** `experiment-launch 383527` → status `running`, flag `single-unit-shipping-surcharge` now active, 50/50 control/surcharge to 100% of traffic (project 480206). Primary = Subtotal per exposed; secondaries = Purchase conversion, Units per exposed, Shipping collected per exposed. Bayesian. Low volume (~1 order/wk) → expect months to read; decide on the contribution insight + breakeven (single-unit conversion drop < ~17%), not on early significance. OPEN: the surcharge UI revamp (badges + tappable meter, announcement bar removed) is still UNCOMMITTED / not deployed, so surcharge-arm users currently see the earlier committed transparency UI until the revamp is committed → merged to main → deployed. Experiment validity is UI-agnostic (charge is constant), but deploy the revamp soon for the intended UX.

**Contribution insight 2026-07-14.** Built the true decision metric as a saved HogQL insight in 480206: "Shipping A/B · Contribution per exposed visitor" (short_id `Z3bd3MOm`). Per arm it computes `contribution = 0.74*subtotal + shipping_collected - 7.50*orders` (g=gross margin, k=postage constants — EDIT to real numbers) divided by exposed visitors (`$feature_flag_called` uniq persons per `$feature_flag_response`). Numerator splits purchases by their own `shipping_variant` prop. The two `timestamp >= '2026-07-14'` filters must be reset to the LAUNCH date so pre-launch test orders drop out. Caveat: raw SQL does not apply PostHog's filter-test-accounts, so internal/test orders show until real traffic dominates. Decision rule: surcharge wins if its `contribution_per_visitor` >= control's; equivalently, single-unit conversion drop < ~17%.

**Metric fix + token check 2026-07-14.** Caught before launch: experiment `383527`'s primary metric was `mean(sum(order_value))` ("Revenue per exposed") — but `order_value` INCLUDES the $5.99 surcharge, so it would mechanically inflate the surcharge arm and rig the result (exactly what the brief forbade). Repointed primary to **`mean(sum(subtotal))` "Subtotal per exposed"** and added three secondaries: Purchase conversion (funnel), Units per exposed (`item_count`), Shipping collected per exposed (`shipping_amount`). Experiment left in `draft`. Token check via `read-data-schema` on 480206: the `purchase` event there already carries `subtotal`/`shipping_amount`/`item_count`/`shipping_variant`, which means production's webhook is on 480206 with the new code (and since client+server share one token, the client flag reads are 480206 too). Launch is gated only on Matt's final confirm of the Vercel Production env, then `experiment-launch 383527`.

**Live test — surcharge charge CONFIRMED 2026-07-13.** Committed to `staging` (`4486d53`), pushed by Matt, Vercel deployed. Delivery customization activated. Function repo initial commit `9a84594` (branch `main`, no remote yet). First checkout test showed Free on a surcharge single = the Function ran and hid $5.99 because the cart's `_shipping_variant` was not `surcharge`. **Root cause was the test procedure, not code:** (a) a persisted cart freezes `_shipping_variant` at its original cartCreate and never re-stamps, and (b) `posthog.featureFlags.override()` does NOT survive `location.reload()`, so overriding-then-reloading stamped `control`. Correct manual-test recipe: `localStorage.clear()` + reload (re-accept age gate) → set the override → verify `getFeatureFlag` prints the arm → add to cart WITHOUT reloading (fresh cartCreate stamps the arm) → checkout. With that, a surcharge single now shows **$5.99** at checkout. Note this gotcha is manual-override-only; when the experiment is live the bucketed variant loads on first page load and stamps correctly with no reload dance.
---

### Rebate landing page — "Show It Off, Get $5 Off" (`/show-it-off`) (2026-07-17)

New campaign landing page for the $5 post-and-share rebate. A customer posts their Litsaber lit up on TikTok/Instagram (tagged `@getlitsaber` + `#Litsaber`), submits the form with their order number + post link, and gets $5 refunded to their original payment method. Figma: desktop `4472:668`, mobile `4476:1065`. Distinct offer from the $10 `WELCOME` promo — do not conflate.

**4-step delivery plan (Matt's framing).** (1) front-end, (2) HubSpot submission + success toast, (3) PostHog instrumentation, (4) ops reporting (pending/approved/complete) + confirmation email.

**Step 1 — front-end ✅ (2026-07-17).** Route `app/show-it-off/page.tsx`. Components in `components/rebate/`: `RebateHero`, `HowItWorks` (4 numbered steps left + `RebateForm` right, mirrors the wholesale two-column CTA layout), `RebateForm`, `RebateFaq` (plus-icon accordion, no numbers), `RebatePageTracker`. Copy/options in `rebate.content.ts`. Reused wholesale conventions verbatim (tokens, FIELD_BASE, motion EASE, pink CTA). Verified at mobile + desktop, full DOM renders, no runtime errors (the transient `NotFoundErrorBoundary` console lines were stale Fast-Refresh-full-reload buffer entries, not live — server log was clean 200s; only 404s were the local-dev Blob images).
- **Brand-voice fix:** Figma step /04 copy ended "That's all you need to do!" (banned exclamation) → rewritten to "That's all there is to it."
- **Mobile "Festival Drop List" section is FREE** — it's the global `EmailSignupBanner`, which `EmailSignupBannerGuard` already renders on every route except `/` and `/policies/*`. Nothing to build; it appears on `/show-it-off` automatically (desktop too, consistent with wholesale/contact).
- **Hero image:** reused `mediaUrl("home/rave.jpg")` (Figma layer was literally `raves_where_it_lives`). Renders blank in local dev (no `NEXT_PUBLIC_MEDIA_BASE_URL`); resolves on Vercel.
- **Design-fidelity pass (2026-07-17, Matt's Figma review):** (1) hero background is `linear-gradient(180deg,#000000 0%,#150C2D 100%)` (was a radial glow); (2) full-width horizontal cyan divider at the hero→section-2 boundary — `linear-gradient(90deg,#150C2D,#00E5FF 48%,#150C2D)`, `h-[2px]`, rendered as the last child of the hero section so it spans edge-to-edge (corrected from an earlier misread that placed it vertically in the gutter); (3) blurred ellipse behind the form in section 2 — 600×600, `#4B2F81` @ 50% opacity, `filter: blur(300px)`, top 238 / right −30; (4) step /04 uses `#EC5793` for its box stroke AND the `/04` number (other steps stay cyan); (5) mobile hero stacks image BELOW the copy+button (`flex-col`, was `flex-col-reverse`); (6) `**bold**` emphasis in the step bodies now renders via a `renderBold` splitter in `HowItWorks` (brighter/semibold white).
- **Popups suppressed on the page (2026-07-17):** added `/show-it-off` to `isSuppressedRoute()` in `lib/hooks/usePromoPopup.ts` so the floating $10 promo popup does not fire on this campaign page. Toasts (form success) intentionally stay.
- **Age gate suppressed on `/show-it-off` (2026-07-17, Matt's call).** Added `/show-it-off` alongside `/activate` in the early-return in `AgeGateModal.tsx`. Rationale + precedent: `/activate` already skips the gate because it's reached by post-purchase customers via the QR on their box; the rebate page is the SAME audience/entry — Matt confirmed the rebate URL only lives on the box QR (not shared publicly), so every visitor is a completed purchaser already 21+ verified at checkout. Same trust model as `/activate`, no wider exposure. The gate still protects every other route (navigating from here to /shop etc. still gates if no cookie).
- **Two more tweaks (2026-07-17):** form name fields reordered to First Name / Last Name (was Last/First); added a second full-width cyan divider (same 90deg gradient) between section 2 and section 3, appended as the last child of the HowItWorks section.

**Step 2 — HubSpot submission ✅ COMPLETE + live-verified (2026-07-17).** `app/api/rebate/route.ts` cloned from `app/api/wholesale/route.ts` (rate-limit → honeypot → Turnstile → v3 submit). Form: portal `244547358`, form `44b281f3-f679-4531-967b-9d9a70062d08` (region na2, Matt-created 2026-07-17). `firstname`/`lastname`/`email` are standard props. The 3 CUSTOM contact-property internal names — **CONFIRMED by Matt**: Order # = `order_number`, Platform = `social_media_platform`, Post URL = `add_link` (env-overridable via `HUBSPOT_REBATE_ORDER_FIELD` / `HUBSPOT_REBATE_PLATFORM_FIELD` / `HUBSPOT_REBATE_POST_URL_FIELD`). ⚠️ A wrong internal name 400s the WHOLE v3 submit. **Live end-to-end submit returned `{ok:true}` / HubSpot 200** — a test contact `rebate.test@getlitsaber.com` was created (delete it in HubSpot). Validation branches tested (missing field → 400, honeypot → silent ok). Env in `.env.example`. This is a NEW embed-type form, so its field defs are not publicly fetchable (v2/v3 form-def endpoints reject it).

**Step 3 — PostHog instrumentation ✅ (2026-07-17).** Added typed events to `lib/analytics/events.ts`: `rebate_page_viewed` (`{source}`), `rebate_form_started` (`{}`), `rebate_form_submitted`, `rebate_submit_error` (`{reason, source}`). `RebatePageTracker` fires page-view once/session via `trackWhenReady` (sessionStorage guard `litsaber_rebate_viewed`). Form fires started on first interaction (`startedRef`), submitted on success (after `identifyByEmail`), error on failure. Funnel = viewed → started → submitted. Events only fire when `NEXT_PUBLIC_POSTHOG_TOKEN` is set (no-op locally).
- **`rebate_form_submitted` enriched (2026-07-17):** now carries the full submission — `{first_name, last_name, email, order_number, platform, post_url, source}` — plus `identifyByEmail(email)` so PostHog stitches the person.
- **`rebate_refund_granted` (server-side, NEW):** fired from the refunds webhook (see Step 4) so PostHog can report total rebates granted (event count) and total value (`sum(refund_amount)`). Not in the client `EVENTS` registry — it's a server event via `posthog-node`, same pattern as the `purchase` event in `orders/route.ts` (raw string name).

**Step 4 — ops reporting + confirmation email (DECIDED 2026-07-17; HubSpot config, no repo code).** Both choices locked by Matt:
- **Status tracking = HubSpot Tickets pipeline.** Stages **Pending → Complete**, plus a **Rejected** stage. (The "Approved" stage was DROPPED 2026-07-17 — approving and refunding happen in one motion, so it added a step with no distinct customer message.) One ticket per submission, opened by a HubSpot workflow on form submission (associate to contact; carry order #, platform, post URL). Ops flow: review the post → if valid, issue the $5 refund in Shopify and move to Complete; if invalid, move to Rejected. "Complete" = the Shopify refund is actually issued.
- **Emails (3, all HubSpot; copy in `docs/rebate-confirmation-email.md`, BRAND.md-compliant):** Email B = submission acknowledgment (HubSpot form follow-up email, immediate). Email A = "$5 refunded" on **ticket → Complete** (workflow). Email C = "could not verify" rejection — sent MANUALLY as a saved template with a fill-in `[REASON]` (reviewed one at a time; reason snippets in the doc), since there's no review agent to auto-render a verdict. Optional later: semi-automate C via a "rejection reason" dropdown property + workflow on ticket → Rejected.
- **Rebate Tickets pipeline ID = `2430290680`** (Matt, 2026-07-17).
- **The refund→email bridge (the open design question).** Completion trigger = Matt refunds the $5 on the Shopify order. HubSpot cannot *hear* a Shopify refund on its own — something must tell HubSpot by moving the ticket to Complete; the email then fires off the stage change. Two paths:
  - **v1 (now, ZERO code):** Matt refunds in Shopify → manually drags the rebate ticket to **Complete** in pipeline 2430290680 → a HubSpot workflow (enrollment: "ticket entered stage Complete") sends the "$5 sent" email. Pragmatic at launch volume. The email's trigger is the HubSpot stage change, which Matt performs alongside the refund.
  - **v2 (later, automated ticket-move):** register a Shopify **`refunds/create`** webhook → match the rebate ticket by order number/email → move it to Complete via the HubSpot CRM API (`PATCH /crm/v3/objects/tickets/{id}`, `hs_pipeline_stage`=Complete stage id). Same workflow fires the email. **Needs a NEW HubSpot private-app token with `tickets` write scope — none exists in the codebase today** (the site only uses HubSpot's public form-submit endpoint). Also needs the Complete stage id. NOT yet built (this is the HubSpot-side automation of the manual drag).
- **Refunds webhook + `rebate_refund_granted` event ✅ built (2026-07-17).** `app/api/webhooks/refunds/route.ts` — HMAC-verified (`SHOPIFY_WEBHOOK_SECRET`, cloned from `orders/route.ts`), listens on Shopify topic **`refunds/create`**. Treats a refund as a rebate payout when its **note contains "rebate"** (preferred, explicit) OR its amount equals **$5.00** (`REBATE_AMOUNT`, catches un-noted refunds but also any unrelated exactly-$5 refund — note it to be safe). Enriches via `getOrderByShopifyId` (new Supabase getter) to recover the stitched `distinct_id` + email from the mirrored order, then fires server-side PostHog `rebate_refund_granted` `{order_id, order_number, refund_amount, currency, matched_by, email, source}`. ⚠️ **Ops must register the `refunds/create` webhook in Shopify → `/api/webhooks/refunds`** (the `orders/create` one is already registered; this is a separate topic). This event is independent of the HubSpot ticket move — it only powers the PostHog rebate-count + total-value reporting Matt asked for.
- **Email copy ✅ drafted** in `docs/rebate-confirmation-email.md` (Email A = "$5 on the way" on ticket→Complete; Email B = optional submission acknowledgment). Brand-voice compliant.
- **Build owner:** HubSpot pipeline/workflow config by Matt in-portal. v2 HubSpot ticket-move webhook is repo code (build when requested; blocked on the HubSpot private-app token + Complete stage id).

**Also (2026-07-17): write-path reversed.** CLAUDE.md updated — Claude Code is now the primary write path, Bolt secondary (was "Bolt is the single write path, 2026-05-23"). The "only one tool writes at a time" invariant is unchanged.
