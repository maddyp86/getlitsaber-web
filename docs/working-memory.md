...
**Fix:** new `components/primitives/ResponsiveMount.tsx` mounts exactly ONE variant via `matchMedia` (mobile-first SSR/first-paint → no hydration mismatch, then upgrades to desktop above 1024px). All five wrappers converted to it, so the off-viewport tree and all its media never enter the DOM. Added `lib/usePlayWhenVisible.ts` (IntersectionObserver play/pause) wired into PartyVideo with `preload="none"` so the largest below-the-fold clip (7.8 MB) only fetches/plays near the viewport. Verified live in the preview: at 375px the homepage mounts 3 videos instead of 5 and the desktop hero (`minHeight:1500px`) is absent from the DOM; at 1280px the desktop variants mount correctly (no regression); type-check clean, no console/hydration warnings. Bolt's earlier tweaks were left in place — harmless, just not load-bearing. (Local-dev note unrelated to the bug: local `node_modules` is missing `@vercel/edge-config` and `pnpm-lock.yaml` is from an incompatible pnpm version — both production-irrelevant.)
